defmodule CaltarWeb.Components.Modal do
  use Phoenix.Component

  alias CaltarWeb.Components.Icon
  ...
  def modal(assigns) do
    ~H"""
    <div class="bg-gray-900 absolute top-0 right-0 w-full max-w-[400px] mt-4 mr-4 rounded-lg shadow-lg transition-all slide-left">
      ...
      <%= with [slot | _] <- @header do %>
        <div class={
          Html.class("text-xl font-bold border-b border-b-gray-800 p-2", Map.get(slot, :class))
        }>
          {render_slot(slot)}
        </div>
      <% end %>
      ...
      <%= with [slot | _] <- @footer do %>
        <div class={Html.class("border-t border-t-gray-800 p-2", Map.get(slot, :class))}>
          {render_slot(slot)}
        </div>
      <% end %>
    </div>
